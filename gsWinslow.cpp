...
real_t gsWinslow::evalObj() const {
    ...
    // FIXIT: Hardcoded for negative determinant
...
gsMatrix<> gsWinslow::hessObj(gsMatrix<> &hessObjTagged) const{
    ...
    gsMatrix<> gsWinslow::hessObj(gsMatrix<> &hessObjTagged) const{
        ...
        A.assemble(
            2*(
                (jac(u)*jac(u).tr()).trace()*detJinv
                - matrix_by_space(jac(G).inv(),jac(u)).trace()*
                matrix_by_space(jac(G).tr(),jac(u)).trace().tr()*detJinv
            )
            //+ (matrix_by_space(jac(G).inv(),jac(u))*(jac(G).inv()*jac(u)).tr()).trace()
            //    *matrix_by_space(jac(G).tr(),jac(u)).trace().trace().val()*detJinv
            );
